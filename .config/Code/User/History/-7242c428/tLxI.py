...
def calculateKabsch(matrix_P, matrix_Q): 
    if inspect.stack()[1][3] == 'inputFiles':
        ...
        if np.linalg.det(matrix_Vt * (np.transpose(matrix_U))) < 0: 
            counter_identitiy = np.array([[1,0,0], [0,1,0], [0,0,-1]])
            matrix_R = (np.transpose(matrix_U)) * counter_identitiy * matrix_Vt
        else:   
        ...
        if np.linalg.det(np.matmul(matrix_Vt, (np.transpose(matrix_U)))) < 0: 
            ...
            matrix_R = np.matmul(, matrix_Vt)
            matrix_R = (np.transpose(matrix_U)) * matrix_Vt
            
        #Finally, calculate the rotated P matrix 
        rot_matrixP = np.matmul(matrix_R, matrix_P)
        rot_matrixP = matrix_R * matrix_P 
